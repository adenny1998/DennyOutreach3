...
/* Denver rule: default at 6 AM; after 4 PM shift to next day 6 AM.
   If a step has waitHours or waitDays, apply them from enrollment moment. */
function computeDueAt(step, enrolledAt, isFirst){
  ...
  // if no wait specified and it's the first step, schedule now rule
  if (isFirst && !step.waitDays && !step.waitHours) {
    ...
    // for waited steps, if resulting time lands past 4 PM, push to next day 6 AM
    if (dt.getHours() >= endH) {
      dt.setDate(dt.getDate()+1);
      dt.setHours(startH,0,0,0);
    } else if (dt.getHours() < startH) {
      dt.setHours(startH,0,0,0);
    }
  }
...
/* Preloaded sequences and steps (email/call only) */
function defaultSequencesAndSteps(){
  ...
  // General Prospecting: Day 1 email, +2h call, then days 3,5,7,9,12
  const general = [
    make(genId, 1, "email", 0, 0),
    make(genId, 2, "call", 0, 2),
    make(genId, 3, "email", 2, 0),  // shows on day 3 at 6am
    make(genId, 4, "call", 4, 0),   // day 5
    make(genId, 5, "email", 6, 0),  // day 7
    make(genId, 6, "call", 8, 0),   // day 9
    make(genId, 7, "email", 11, 0), // day 12
  ];

  // GoFish: same as General, editable
  const gofish = general.map(s => ({ ...s, id: makeId(), sequenceId: fishId }));

  // Nurture 45 days: light touches
  const nurture = [
  ...
  steps = steps
    ...
    .map((st, idxPerSeq => st)); // placeholder to keep order

  steps = steps.map(st => ({
    ...st,
    name: `${byId[st.sequenceId]?.name || "Sequence"} - Day ${st.waitDays || 0}${st.waitHours?` + ${st.waitHours}h`:``} - ${st.actionType}`
  }));
...
const initialStore = (() => {
  ...
  return {
    ...
    steps: base.steps,
    tasks: [], // { id, contactId, sequenceId, stepId, actionType, name, dueAt, outcome, notes }
  };
})();
...
function SequencesView({ store, setStore }){
  ...
  function removeStep(id){
    setStore(prev => ({ ...prev, steps: prev.steps.filter(st=>st.id!==id) }));
  }

  const steps = store.steps
    .filter(st => st.sequenceId===selectedId)
    .sort((a,b)=> a.order - b.order);
...
/* Task table with wide, auto-expanding notes and company/contact notes surfaced */
